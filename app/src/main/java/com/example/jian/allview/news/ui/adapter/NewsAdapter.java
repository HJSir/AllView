...
import com.example.jian.allview.utils.TimeUtils;
...
public class NewsAdapter extends BaseQuickAdapter<News, BaseViewHolder> {
     ...
     * @param data        新闻集合
     */
    public NewsAdapter(Context context, boolean isVideoList, List<News> data) {
        super(data);
        mContext = context;
...
//前面的lSysTime是秒数，先乘1000得到毫秒数，再转为java.util.Date类型
            Date dt = new Date(news.behot_time  * 1000);
            String sDateTime = sdf.format(dt);
//            Log.i("s",DateUtils.getShortTime(news.behot_time * 1000));
...
//        boolean isTop = position == 0 && mChannelCode.equals(channelCodes[0]); //属于置顶
        ...
        helper.setVisible(R.id.tv_tag,  isHot || isAD);//如果是上面任意一个，显示标签
